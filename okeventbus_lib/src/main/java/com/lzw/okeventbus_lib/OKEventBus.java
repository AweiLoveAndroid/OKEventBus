...
/**
 * OKEventBus is a central publish/subscribe event system for Android. Events are posted ({@link #post(Object)}) to the
 ...
 * (the event).
 *
 * 这个库是在原有EventBus基础上改造而成的，主要解决了EventBus不好维护的难题。
 *
 * @author Markus Junginger, greenrobot，lzw
 *
 */
public class OKEventBus {

    /** Log 标记 */
    public static String TAG = OKEventBus.class.getSimpleName();

    static volatile OKEventBus defaultInstance;
    ...
    /** EventBus的单例. */
    public static OKEventBus getDefault() {
        if (defaultInstance == null) {
            synchronized (OKEventBus.class) {
                if (defaultInstance == null) {
                    defaultInstance = new OKEventBus();
                }
            }
        }
    ...
    public String toString() {
        return "OKEventBus[indexCount=" + indexCount + ", eventInheritance=" + eventInheritance + "]";
    }
}
